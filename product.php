...
$s = getAllSettings();
$g = function($key, $default = '') use ($s) { return $s[$key] ?? $default; };
...
<div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
    ...
    <div class="grid lg:grid-cols-2 gap-10">
        ...
        <!-- Info -->
        <div>
            <h1 class="text-2xl lg:text-3xl font-serif font-bold text-elite-800 mb-3 italic"><?= $product['name'] ?></h1>
            ...
            <div class="bg-elite-50 rounded-2xl p-5 mb-6">
                <h3 class="text-sm font-bold text-elite-700 uppercase tracking-wide mb-3">📦 ข้อมูลสินค้า</h3>
                <div class="space-y-2.5">
                    <?php if ($product['sku']): ?>
                        <div class="flex items-center gap-3 text-sm text-elite-500"><span>🏷️</span> SKU: <?= $product['sku'] ?></div>
                    <?php endif; ?>
                    <div class="flex items-center gap-3 text-sm text-elite-500"><span>📂</span> หมวดหมู่: <?= $product['category_name'] ?></div>
                    <div class="flex items-center gap-3 text-sm text-elite-500"><span>✨</span> สั่งทำพิเศษตามขนาดและดีไซน์ที่ต้องการ</div>
                </div>
            </div>

            <!-- Contact Social Links -->
            <div class="bg-elite-50 rounded-2xl p-5">
                <h3 class="text-sm font-bold text-elite-700 uppercase tracking-wide mb-3">💬 สนใจสินค้า? ติดต่อเราได้เลย</h3>
                <div class="grid grid-cols-1 sm:grid-cols-3 gap-3">
                    <a href="<?= $g('social_line_url', '#') ?>" target="_blank"
                        ...
                        <svg class="w-5 h-5 flex-shrink-0" viewBox="0 0 24 24" fill="currentColor">
                            <path d="M24 10.304c0-5.369-5.383-9.738-12-9.738-6.616 0-12 4.369-12 9.738 0 4.814 4.269 8.846 10.036 9.608.391.084.922.258 1.057.592.121.303.079.778.039 1.085l-.171 1.027c-.053.303-.242 1.186 1.039.647 1.281-.54 6.911-4.069 9.428-6.967C23.309 14.254 24 12.382 24 10.304z"/>
                        </svg>
